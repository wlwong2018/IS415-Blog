...
- **Locational Factors**:
    ...
    + *Number of kindergartens within 350m*: It is right-skewed, implying that more HDB flats have less kindergartens within 350m.
    + *Number of childcare centres within 350m*: It is right-skewed, implying that more HDB flats have less childcare centres within 350m.
    + *Number of bus stops within 350m*: It is slightly right-skewed, implying that slightly more HDB flats have less bus stops within 350m.
    + *Number of primary schools within 1km*: It is slightly right-skewed, implying that slightly more HDB flats have less primary schools within 1km. 
...
- *corrplot()* of **corrplot** package to plot a correlation plot
- The argument *order* is using **AOE** where it orders the variables by using the angular order of the eigenvectors method suggested by [Michael Friendly](https://www.datavis.ca/papers/corrgram.pdf)

```{r echo=TRUE, eval=TRUE, fig.width=12, fig.height=12}
...
- None of the variables are highly correlated with each other since they are all **lower than 0.7**.
- The threshold of [highly correlated is >= 0.7](https://www.andrews.edu/~calkins/math/edrm611/edrm05.htm#:~:text=Correlation%20coefficients%20whose%20magnitude%20are,can%20be%20considered%20highly%20correlated.)
- We will hence not remove any variables
...
- *summary()* of **Base R** to print summary statistics of model
...
- *lvl_19_TO_21* and *lvl_22_TO_24* are **not statistically significant**. 
- We will hence revise the model by removing these variables.
...
- *ls_vif_tol() *of **olsrr** package is used to test if there are sign of multicollinearity.
...
- There are **no signs of multicollinearity** among the independent variables since the VIF of the independent variables are **less than 10**

### 8.1.4 Test for Non-Linearity

- Secondly, we also have to ensure that the **dependent** and **independent** variables are **approximately linear**.
- We can use *ols_plot_resid_fit()* of **olsrr** package is used to perform linearity assumption test.
...
- Thirdly, the **residuals** are assumed to be **normally distributed**.
- We can use *ols_plot_resid_hist()* of **olsrr** package to perform the normality assumption test.
...
- The residual of the multiple linear regression model (i.e. resale.mlr1) resembles **normal distribution**.
...
- *as.data.frame()* of **Base R** to export the residual of the hedonic pricing model and save it as a data frame.
...
- Join the newly created data frame with resale_sf object.
...
- *as_Spatial()* of **sp** package to convert resale.res.sf simple feature object into a SpatialPointsDataFrame because spdep package can only process sp conformed spatial data objects
...
- There are **signs** of **spatial autocorrelation**.
- To proof that our observation is indeed true, the Moran’s I test will be performed.

### 8.1.6.5 Compute distance-based weight matrix

- We first have to use *dnearneigh() *of **spdep** package to compute the distance-based weight matrix
...
- Then, *nb2listw()* of **spdep** packge will be used to convert the output neighbours lists (i.e. nb) into a spatial weights
...
- Use *lm.morantest()* of **spdep** package to perform Moran's I Test for residual spatial autocorrelation
...
- Since the Observed Global Moran I = *0.3815577* which is *greater than 0*, we can infer that the **residuals resemble cluster distribution**.
...
- *bw.gwr()* of **GWmodel** package is used to determine the optimal adaptive bandwidth to use in the model. 
- The argument *kernel* is set to **gaussian** since in **Section 4.13**, we saw that data points are spread across the whole of Singapore.
- The argument *adaptive* is set to **TRUE** as we are now interested to compute the adaptive bandwidth.
...
- **45** is the recommended bandwidth to use in the constructing of adaptive bandwidth gwr model.


### 9.1.2 Constructing the adaptive bandwidth gwr model

- *gwr.basic()* of **GWmodel** package to calibrate the gwr model using adaptive bandwidth and gaussian kernel.
...
- The adjusted r-square value of **gwr** is **0.925323** which is **significantly better** than the global multiple linear regression model of **0.6471**.
...
- *Condition Number*: evaluates **local collinearity**. In the presence of strong local collinearity, the results become unstable. Results associated with condition numbers larger than 30, may be unreliable.

- *Local R2*: values range between 0.0 and 1.0 and indicate **how well the local regression model fits observed y values**. Low values indicate the local model is performing poorly.
...
- *summary()* of **Base R** to reveal the summary statistics of the predicted value of resale price, denoted as yhat.
...
- The lowest predicted value of a 4-room HDB resale price is $230,357
- The mean predicted value of a 4-room HDB resale price is $405,707  
- The highest predicted value of a 4-room HDB resale price is $990,368 
...
- A few areas in the East and North-East region are a lighter shade, implying that the GWR model is unable to predict well in these areas.
- However, **majority** of the Local_R2 values are **within 0.6 to 1.0** since it is in a darker shade. Hence, we can conclude that the 4-room HDB resales prices **can be explained** by the GWR model.
...
[Correlation](https://www.andrews.edu/~calkins/math/edrm611/edrm05.htm#:~:text=Correlation%20coefficients%20whose%20magnitude%20are,can%20be%20considered%20highly%20correlated.)

[Downtown Core](https://www.latlong.net/place/downtown-core-singapore-20616.html)

[epsg.io](https://epsg.io/3414)

[Geographically Weighted Regression](https://gdsl-ul.github.io/san/gwr.html)

[Geographically Weighted Regression Exercise](https://is415.netlify.app/hands-on_ex/hands-on_ex09/hands-on_ex09-gwr)

[Gifted Education Programme](https://www.moe.gov.sg/programmes/gifted-education)

[gps-coordinates.net](https://www.gps-coordinates.net/map/country/SG)

[Michael Friendly](https://www.datavis.ca/papers/corrgram.pdf)

[rowMins](https://www.rdocumentation.org/packages/rje/versions/1.10.16/topics/rowMins)
