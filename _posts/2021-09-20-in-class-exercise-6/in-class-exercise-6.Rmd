...
description: |
  This is a continuation of Hands-on Exercise 6. 
author:
  - name: Wong Wei Ling
    url: www.google.com
date: 09-20-2021
...
- Insert text in map
- Add in fig.width and fig.height to change the figure size so that it will not be covered by the legend.
...
- Link number distribution:
    ...
    + 2 polygons with 1 neighbour
    + 12 polygons with 3 neighbours
    + etc.
...
**Added for ICE 6**
- To retrieve GDPPC of these 5 countries. 
...
- Our function will be st_centroid.(calculate centroid of each polygons aka each of the 88 regions)
...
```{r eval=TRUE, echo = TRUE}
longitude <- map_dbl(hunan$geometry, ~st_centroid(.x)[[1]]) # x-axis
...
```{r eval=TRUE, echo = TRUE}
latitude <- map_dbl(hunan$geometry, ~st_centroid(.x)[[2]]) # y-axis
...
- Ensures that it falls inside the polygon.
- Centroid: measures from the center point and not the side. 
- Queen's will have more neighbours than Rooks because of the rules. 
...
- If unprojected coordinates are used and either specified in the coordinates object x or with x as a two column matrix and **longlat=TRUE (so they know that it is in decimal degree and will assume that it is in WGS84), great circle distances in km will be calculated assuming the WGS84 reference ellipsoid.**

## 3.4.1 Determine the cut-off distance

- To determine the upper limit for distance band:

    + Return a matrix with the indices of points belonging to the set of the k nearest neighbours of each other by using *knearneigh()* of **spdep** package.
...
- Min & max is based on the code chunk above. 
- Min = 0 so that it would look like a donut. (will not look like a circle)
- Max: 61.27, so we specify as 62.
...
- Of all regions, the average number of links for all regions are approximately 4. 
...
- we use adaptive because sometimes the entites might be very close/ far apart.
- This code is to look for the 4 closest neighbours. 
...
ids <- lapply(dist, function(x) 1/(x)) # x indicares the distance
ids
```
...
- nb2listw() supplements a neighbours list with spatial weights.

- style: 

    + b returns binary
    + See https://r-spatial.github.io/spdep/reference/nb2listw.html for more information
    
- **The zero.policy=TRUE option allows for lists of non-neighbors. Default is NULL. FALSE: not getting any location with 0 neighbours - so at the start you must already exclude then it can be FALSE**
...
- Recall in previous section, we retrieve the GDPPC of rhese 5 countries using the code chunk below. 
...
- ^ Add all the numbers and divide 5, you will get 24847.20 in GDPPC.lag.

<br>

- We can append the spatially lag GDPPC values onto hunan SpatialPolygonDataFrame
...
- 1 of the center region of Lag is the lowest(gdppc) compared to the same region in the GDPPC graph because it is much smaller than its neighbours.(see based on colour)
