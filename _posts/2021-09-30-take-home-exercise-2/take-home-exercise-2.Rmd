...
- To investigate if the distribution of Airbnb listings are affected by location factors such as existing hotels, MRT services and tourist attractions. 
...
- Hotels, tourist attractions, MRT services were supposed to be extracted from SLA OneMap Service by using onemapsgapi. However, due to some issues of creating a onemap account, Prof. Kam has kindly provided us with the data in CSV format. 
...
- The geometry type is point, multipolygon and polygon for the mrt_sf, mpsz_sf and sg_sf data set.
...
- *st_make_valid()* to make the geometries valid for the *mpsz_sf* and *sg_sf* data set only since MRT has no invalid geometries
...
We have to check for NA values because if NA values are not properly handled, it might hinder with the analysis in the later steps. Additionally, we have to convert them to sf objects and assign them the right CRS, making these data sets suitable to be plotted on a map. 
...
- *glimpse()* is used to look into the data sets.
...
- NA values in other columns do not have to be dropped as the *data* will be dropped evetually when converting Spatial* data frame into Spatial* objects.
...
- *st_as_sf()* of **sf** package is used to convert the the 4 data frames into sf data frames.
- The values in the latitude & longitude columns are in decimal degrees, hence, this means that it is using WGS84. 
- We then need to assign the epsg code of 4326 using *st_transform()* of **sf** package.
...
- *as_Spatial()* of **sf** package is being used to convert the geospatial data from simple feature data frame to sp’s Spatial* class.
...
- Look into the summary of the newly created ppp objects
...
- *any()* and *duplicated()* are used to ensure that there are no duplicated points after using *rjitter()*. The output will return FALSE, if there are no duplicated points, else, it will return TRUE.
- From the previous step, even though *mrt_ppp* object is being shown that there are 0 duplicated points, it is still a good practice to verify using *any()* and *duplicated()*.
...
- It is a good practice to confine the spatial point patterns analysis within a geographical area such as the Singapore boundary.
- In **spatstat** package, this polygonal region is known as **owin**, an object specially designed to represent this boundary.

- *as()* with the argument *owin* to convert *sg* SpatialPolygon object to owin object of **spatstat** package 
...
- *rescale()* of **BaseR** is used to convert the KDE values from metre to kilometre.
...
- Airbnb listings are generally less concentrated in the same area.
...
- Distribution above will be the same since only the units have been converted to meters instead of kilometers. 
...
- *tm_basemap()* of  **tmap** package to specify the visualization to be in OpenStreetMap
...
- Use function created in *Section 7.1.1.3* to plot the map.
...
- Shades of red represent Hotels while shades of blue represent Airbnb listings in 2019. .
...
- Shades of red represent Tourism while shades of blue represent Airbnb listings in 2019.
- Similar to the tmap of Airbnb Listings 2019 against Hotels, it can also be observed that the South-East region of Singapore has a high number of tourist attractions and is therefore dense since the red is darker in this region. 
- However, for tourism, it can be seen that it is slightly more concentrated in the middle as compared to hotels which is slightly more spread out in the previous plot. 
...
- Shades of red represent MRT services while shades of blue represent Airbnb listings in 2019. .
...
- MRT services is in fact more spread out than hotels and tourists attractions within the South-East region of Singapore.
...
- Create function for creating owin objects for each sub zones.
...
#### 7.2.3.2 Perform Complete Spatial Randomness (CSR) test on Cross L-Function
...
- The hypothesis and test are as follows:
    ...
    + H1 = The distribution of Airbnb listings in 2019 and  Hotels/Tourist Attractions/MRT services are **NOT** spatially independent.
...
- For Airbnb listings 2019 vs Hotels
    ...
    + After the **150m mark**, the black line is below the red line and this shows signs of regular pattern. However, we can only say that it shows sign of significant regular pattern **after the 160m mark** as this is the part where the black line is below the envelope. Since it is below the envelope, we have enough statistical evidence to **reject the null hypothesis** at 90% confidence interval.
    + To summarise, for both distances i.e.50 to 150m and after the 160m mark, are NOT spatially independent but from **50 to 150m**, it has **significant clustered patterns** while after the **160m mark**, it has **significant regular pattern**.

- For Airbnb listings 2019 vs Tourism

    + The black line is below the red line which shows signs of regular pattern. But it is only significant for distances **after 250m**.
    + Hence, only for distances **after 250m**, we have enough statistical evidence to **reject the null hypothesis** at 90% confidence interval. We can hence infer that it is NOT spatially independent and for distances **after 250m** and it has **significant regular patterns.**

- For Airbnb listings 2019 vs MRT

    + The black line is above the red line which shows signs of clustered pattern. But it is only significant at from **160m to 220m**.
    + Hence, only from the distance of **160m to 220m**, we have have enough statistical evidence and we **will reject the null hypothesis** at 90% confidence interval. We can hence infer that it is NOT spatially independent for from the distance of **160m to 220m** and it has **significant clustered patterns.**
...
This section is to analyse the impact of COVID-19 on Airbnb business in Singapore by comparing Airbnb listings data on June 2019 and June 2021. To determine the objective, we will first perform and plot **Kernel Density Estimation maps** by room type then confirm the insights found by using **Second-Order Spatial Point Patterns Analysis** to draw statistical conclusions.

## 8.1 Exploratory Spatial Data Analysis

This section is to analyse the impact of COVID-19 on Airbnb business in Singapore by comparing Airbnb listings data on June 2019 and June 2021. To determine the objective, we will first compute and plot Kernel Density Estimation maps then confirm the insights found by using Second-Order Spatial Point Patterns Analysis to draw statistical conclusions.

### 8.1.1 Kernel Density Estimation

- KDE has been explained in Section 7.1.1

#### 8.1.1.1 Retrieve categories of room_types

- *unique()* to find out the categories of room types in Airbnb Listings in 2019 & 2021.

```{r echo=TRUE, eval=TRUE}
unique(listings_2019_sf$room_type)
unique(listings_2021_sf$room_type)

```

From the results above,

- There are 3 categories in Airbnb Listings in 2019, namely Private room, Entire home/apt and Shared room
- There are 3 categories in Airbnb Listings in 2021, namely Private room, Entire home/apt, Shared room and Hotel room.
- Since **Hotel room** is not available in Airbnb listings in 2019, it will be **excluded** from the analysis. 

#### 8.1.1.2 Subset categories into different ppp objects

- *subset()* to retrieve the ppp objects of the different categories for both Airbnb Listings in 2019 and 2020. 
- This is for calculating and plotting KDE maps in later part of the section.

```{r echo=TRUE, eval=TRUE}
listings_2019_ppp_pr <- subset(listings_2019_ppp, listings_2019_sf$room_type == "Private room")
listings_2019_ppp_ea <- subset(listings_2019_ppp, listings_2019_sf$room_type == "Entire home/apt")
listings_2019_ppp_sr <- subset(listings_2019_ppp, listings_2019_sf$room_type == "Shared room")

listings_2021_ppp_pr <- subset(listings_2021_ppp, listings_2021_sf$room_type == "Private room")
listings_2021_ppp_ea <- subset(listings_2021_ppp, listings_2021_sf$room_type == "Entire home/apt")
listings_2021_ppp_sr <- subset(listings_2021_ppp, listings_2021_sf$room_type == "Shared room")

```

####  8.1.1.3 Re-scale KDE Values

- However, since the default unit of measurement of SVY21 is in metre, we have to convert the KDE values to km so that the values would not be too small to look for useful insights.
- *rescale()* of **BaseR** is used to convert the KDE values from metre to kilometre.

```{r echo=TRUE, eval=TRUE}
listings_2019_ppp_pr.km <- rescale(listings_2019_ppp_pr, 1000, "km")
listings_2019_ppp_ea.km <- rescale(listings_2019_ppp_ea, 1000, "km")
listings_2019_ppp_sr.km <- rescale(listings_2019_ppp_sr, 1000, "km")

listings_2021_ppp_pr.km <- rescale(listings_2021_ppp_pr, 1000, "km")
listings_2021_ppp_ea.km <- rescale(listings_2021_ppp_ea, 1000, "km")
listings_2021_ppp_sr.km <- rescale(listings_2021_ppp_sr, 1000, "km")
```


#### 8.1.1.4 Plot KDE maps using automatic bandwidth methods (ppl)

- Explanation for this section is similar to 7.1.1.1 

##### *bw.ppl*

###### Airbnb Listings 2019

- The function used was created in *Section 7.1.1.1*, to plot KDE maps using the bandwidth *bw.ppl()*
- par() is used to arrange the maps in a side-by-side manner.

```{r echo=TRUE, eval=TRUE, fig.width=10, fig.height=8}
par(mfrow=c(2,2))

plot(kde_ppl(listings_2019_ppp_pr.km), main = "listings 2019 private room bw.ppl")
plot(kde_ppl(listings_2019_ppp_ea.km), main = "listings 2019 entire home/apt  bw.ppl")
plot(kde_ppl(listings_2019_ppp_sr.km), main = "listings 2019 shared room bw.ppl")

```

Results above show that: 

- There is a concentrated area for Shared Rooms and a few concentrated spots for Private Room and Entire room/apt. 


###### Airbnb Listings 2021
...
plot(kde_ppl(listings_2021_ppp_pr.km), main = "listings 2021 private room bw.ppl")
plot(kde_ppl(listings_2021_ppp_ea.km), main = "listings 2021 entire home/apt  bw.ppl")
plot(kde_ppl(listings_2021_ppp_sr.km), main = "listings 2021 shared room bw.ppl")
...
- Likewise, there is a concentrated area for Shared Rooms and a few less concentrated spots for Private Room and Entire room/apt.
- The Entire room/apt in 2021 looks more concentrated than its counterpart in 2019.

##### Reveal density

- *intensity()* of **spatstat** package is used to reveal the density of Airbnb listings in 2019, hotels, tourism and MRT services.

```{r echo=TRUE, eval=TRUE}
intensity(listings_2019_ppp_pr.km)
intensity(listings_2019_ppp_ea.km)
intensity(listings_2019_ppp_sr.km)

intensity(listings_2021_ppp_pr.km)
intensity(listings_2021_ppp_ea.km)
intensity(listings_2021_ppp_sr.km)

```

Results above show that:

- Entire home/apt listings in 2019 have the highest density 5.69 units per km square, followed by Private Room then Shared Room with a density of 4.78 and 0.59 units per km square respectively.
- Private Room 2021 have the highest density of 2.73, followed by Entire home/apt then Shared Room with a density of 2.42 and 0.23 units per km square respectively.


#### 8.1.1.5 Data Preparation for visualising Kernel Density Maps on openstreetmap of Singapore


##### 8.1.1.5.1 Compute KDE maps with metres instead of kilometers

- Since OpenStreetMap's default units is in metres, we have to compute the KDE in metres, instead of using the previously calculated KDE in kilometres. 
- Compute KDE with bw.ppl() using metres

```{r echo=TRUE, eval=TRUE}
kde_listings_2019_ppl_pr <- density(listings_2019_ppp_pr, sigma=bw.ppl, edge=TRUE, kernel="gaussian") 
kde_listings_2019_ppl_ea <- density(listings_2019_ppp_ea, sigma=bw.ppl, edge=TRUE, kernel="gaussian") 
kde_listings_2019_ppl_sr <- density(listings_2019_ppp_sr, sigma=bw.ppl, edge=TRUE, kernel="gaussian") 

kde_listings_2021_ppl_pr <- density(listings_2021_ppp_pr, sigma=bw.ppl, edge=TRUE, kernel="gaussian") 
kde_listings_2021_ppl_ea <- density(listings_2021_ppp_ea, sigma=bw.ppl, edge=TRUE, kernel="gaussian") 
kde_listings_2021_ppl_sr <- density(listings_2021_ppp_sr, sigma=bw.ppl, edge=TRUE, kernel="gaussian") 

kde_listings_2021_ppl <- density(listings_2021_ppp, sigma=bw.ppl, edge=TRUE, kernel="gaussian") 

```


##### 8.1.1.5.2 Convert KDE output into grid object

- For suitability of mapping.
- grid.arrange() of gridExtra package to arrange the grids side-by-side

```{r echo=TRUE, eval=TRUE}
gridded_kde_listings_2019_ppl_pr <- as.SpatialGridDataFrame.im(kde_listings_2019_ppl_pr)
gridded_kde_listings_2019_ppl_ea <- as.SpatialGridDataFrame.im(kde_listings_2019_ppl_ea)
gridded_kde_listings_2019_ppl_sr <- as.SpatialGridDataFrame.im(kde_listings_2019_ppl_sr)

gridded_kde_listings_2021_ppl_pr <- as.SpatialGridDataFrame.im(kde_listings_2021_ppl_pr)
gridded_kde_listings_2021_ppl_ea <- as.SpatialGridDataFrame.im(kde_listings_2021_ppl_ea)
gridded_kde_listings_2021_ppl_sr <- as.SpatialGridDataFrame.im(kde_listings_2021_ppl_sr)

gridded_kde_listings_2021_ppl <- as.SpatialGridDataFrame.im(kde_listings_2021_ppl)

```

##### 8.1.1.5.3 Convert gridded output into raster

- Use *raster()* of **raster** package to convert the gridded kernel density objects to RasterLayer object. 
- Print 1 raster object as an example

```{r echo=TRUE, eval=TRUE}
kde_listings_2019_ppl_raster_pr <- raster(gridded_kde_listings_2019_ppl_pr)
kde_listings_2019_ppl_raster_ea <- raster(gridded_kde_listings_2019_ppl_ea)
kde_listings_2019_ppl_raster_sr <- raster(gridded_kde_listings_2019_ppl_sr)

kde_listings_2021_ppl_raster_pr <- raster(gridded_kde_listings_2021_ppl_pr)
kde_listings_2021_ppl_raster_ea <- raster(gridded_kde_listings_2021_ppl_ea)
kde_listings_2021_ppl_raster_sr <- raster(gridded_kde_listings_2021_ppl_sr)

kde_listings_2021_ppl_raster <- raster(gridded_kde_listings_2021_ppl)

kde_listings_2019_ppl_raster_pr
```

From the results above,

- The CRS is NA, hence we have to assign a projection system to it.

##### 8.1.1.5.4 Assign projection systems

- Assign the epsg code of 3414

```{r echo=TRUE, eval=TRUE}
projection(kde_listings_2019_ppl_raster_pr) <- CRS("+init=EPSG:3414")
projection(kde_listings_2019_ppl_raster_ea) <- CRS("+init=EPSG:3414")
projection(kde_listings_2019_ppl_raster_sr) <- CRS("+init=EPSG:3414")

projection(kde_listings_2021_ppl_raster_pr) <- CRS("+init=EPSG:3414")
projection(kde_listings_2021_ppl_raster_ea) <- CRS("+init=EPSG:3414")
projection(kde_listings_2021_ppl_raster_sr) <- CRS("+init=EPSG:3414")

projection(kde_listings_2021_ppl_raster) <- CRS("+init=EPSG:3414")

```


##### 8.1.1.5.5 Visualise output in openstreetmap

###### Plot Airbnb Listings 2019 vs 2021

- Use function created in *Section 7.1.1.1* to plot on OpenStreetMap Singapore.

```{r echo=TRUE, eval=TRUE}
oms(kde_listings_2019_ppl_raster, kde_listings_2021_ppl_raster, "Airbnb Listings 2019", "Airbnb Listings 2021")
...
- Shades of red represent Airbnb Listings in 2019 while shades of blue represent Airbnb Listings in 2021.
- In general (without specifying the room types), the Airbnb listings in 2021 has fallen from Airbnb listings in 2019. 
- This can be observed by having less shades of blue as compared to more shades of red.
- In 2021, the more prominent areas are in and around Chinatown, Bendemeer as well as near Geylang. 
- This is a significant decrease from 2019 where most parts of the Southern-Eastern region of Singapore are coloured red, representing more number of Airbnb listings in 2019.


###### Plot Private Room 2019 vs 2021

- Use function created in *Section 7.1.1.1* to plot on OpenStreetMap Singapore.

```{r echo=TRUE, eval=TRUE}
oms(kde_listings_2019_ppl_raster_pr, kde_listings_2021_ppl_raster_pr, "Private Room 2019", "Private Room 2021")
...
- For private rooms in general, it can be seen that there are not many of them. 
- But, it can be observed that there were private rooms listings in more areas in 2019 as compared to 2021.
- In 2019, private room listings can be seen in various areas such as Chinatown, near Somerset, Selegie, Bendemeer, several places near Aljunied and more. 
- However in 2021, private room listings can only be seen in Chinatown, near Bencoolen and Bendemeer.
- Hence, there are fewer areas with Private listings in 2021 as compared to 2019.


###### Plot Entire/Apt Room 2019 vs 2021

- Use function created in *Section 7.1.1.1* to plot on OpenStreetMap Singapore.

```{r echo=TRUE, eval=TRUE}
oms(kde_listings_2019_ppl_raster_ea, kde_listings_2021_ppl_raster_ea, "Entire home/apt 2019", "Entire home/apt 2021")
...
- There are more listings for Entire home/apartment in general, as compared to private rooms in the previous map.
- Entire home/apartment listings in 2019 and 2021 are generally in the same area, with 2021 having slightly more listings in other areas.


###### Plot Shared Room 2019 vs 2021

- Use function created in Section A to plot on OpenStreetMap Singapore.

```{r echo=TRUE, eval=TRUE}
oms(kde_listings_2019_ppl_raster_sr, kde_listings_2021_ppl_raster_sr, "Shared Room 2019", "Shared Room 2021")
...
- Shared room listings in general are more concentrated within areas in Southern-Eastern region of Singapore such as People's Park and Little India.
- In 2019, it is observed that the prominent spots are within People's Park as well as between Little India and Kallang area. 
- In 2021 however, it is more concentrated within the the area of Little India and Bugis. 
- This shows that there have been less shared room listings in People's park area and many more in Little India and Bugis area in 2021.

<br>

**Conclusion for Section B's Exploratory Spatial Data Analysis:**

- With kernel density maps however, even though we do see that the changes in distribution of Private Room, Entire home/apt and Shared Room from 2019 to 2021, we cannot conclude for sure that the distribution of these Airbnb listings room types are impacted by COVID-19. 
- Hence, we need to employ **Second-order Spatial Point Patterns Analysis** to draw statistical conclusions. 


## 8.2 Second Order Spatial Point Patterns Analysis 

#### 8.2.1 Combine SpatialPoints objects with owin objects

- To only extract point events from the Downtown Core region.

```{r echo=TRUE, eval=TRUE}
dc_2019_pr <- listings_2019_ppp_pr[downtowncore_owin]
dc_2021_pr <- listings_2021_ppp_pr[downtowncore_owin]

dc_2019_ea <- listings_2019_ppp_ea[downtowncore_owin]
dc_2021_ea <- listings_2021_ppp_ea[downtowncore_owin]

dc_2019_sr <- listings_2019_ppp_sr[downtowncore_owin]
dc_2021_sr <- listings_2021_ppp_sr[downtowncore_owin]

```


### 8.2.2 Point Patterns Analysis: L-Function

- Explanation for using L-Function is similar to Section A.

#### 8.2.2.1 Compute L-Function Estimation

- Create a function for computing L-Function Estimation

    + *Lest()* of **spatstat** package is used to compute the L-Function estimation.
    + *plot()* to visualise the relationship.
    
- Finally, the function created is being called to plot the graphs.

```{r echo=TRUE, eval=TRUE, fig.width=12, fig.height=8}
l_test <- function(data, title) {
  compute <- Lest(data, correction='Ripley')
  
  plot(compute, . - r ~ r, xlab="d", ylab="L(d)-r", main=title)
}

par(mfrow=c(3,2))
l_test(data = dc_2019_pr, title="Listings 2019 PR Lcross")
l_test(data = dc_2021_pr, title="Listings 2021 PR Lcross")

l_test(data = dc_2019_ea, title="Listings 2019 EA Lcross")
l_test(data = dc_2021_ea, title="Listings 2019 EA Lcross")

l_test(data = dc_2019_sr, title="Listings 2019 SR Lcross")
l_test(data = dc_2021_sr, title="Listings 2019 SR Lcross")
...
- There is a sign that the marked spatial point events are not randomly distributed at certain distances for all plots except for Shared Room in 2021.
- However, a hypothesis test is still required to confirm this observation statistically.


#### 8.2.2.2 Perform CSR test on L-Function

- The hypothesis and test are as follows:

    + Ho = The distribution of Airbnb listings in 2019/2021 by room type are randomly distributed.
    + H1 = The distribution of Airbnb listings in 2019/2021 by room type are **NOT** randomly distributed.
    
- The null hypothesis will be rejected if p-value is smaller than alpha value of 0.1 (i.e. at 90% confident interval).

#### 8.2.2.3  Monte Carlo test with L-Function

- *envelope()* of **spatstat** package is used to perform CSR test
- A total of 100 simulations will be ran. Using **spatstat** package, the end is always 1 less as the 1st simulation is 0 (and not 1), hence in the argument *nsim*, it is defined as 99 and not 100. 
- A global *set.seed()* is used to ensure that the same output is computed each time the code is being ran. (*set.seed(1234)* is being used and it is placed at the top of .RMD file)

```{r echo=TRUE, eval=TRUE}

dc_listings_2019_pr_L.csr <- envelope(dc_2019_pr, Lest, nsim = 99, rank = 1, global=TRUE)
dc_listings_2021_pr_L.csr <- envelope(dc_2021_pr, Lest, nsim = 99, rank = 1, global=TRUE)

dc_listings_2019_ea_L.csr <- envelope(dc_2019_ea, Lest, nsim = 99, rank = 1, global=TRUE)
dc_listings_2021_ea_L.csr <- envelope(dc_2021_ea, Lest, nsim = 99, rank = 1, global=TRUE)

dc_listings_2019_sr_L.csr <- envelope(dc_2019_sr, Lest, nsim = 99, rank = 1, global=TRUE)
dc_listings_2021_sr_L.csr <- envelope(dc_2021_sr, Lest, nsim = 99, rank = 1, global=TRUE)

```


#### 8.2.2.4  Plot L-Function

- plot() is used to plot the simulated L-Function

```{r echo=TRUE, eval=TRUE, fig.width=12, fig.height=8}
par(mfrow=c(3,2))

plot(dc_listings_2019_pr_L.csr, . -r ~ r, xlab = "distance(m)")
plot(dc_listings_2021_pr_L.csr, . -r ~ r, xlab = "distance(m)")

plot(dc_listings_2019_ea_L.csr, . -r ~ r, xlab = "distance(m)")
plot(dc_listings_2021_ea_L.csr, . -r ~ r, xlab = "distance(m)")

plot(dc_listings_2019_sr_L.csr, . -r ~ r, xlab = "distance(m)")
plot(dc_listings_2021_sr_L.csr, . -r ~ r, xlab = "distance(m)")
...
- For both years, the black line (observed curve) is well above the red line (theoretical curve) as well as the envelope after about 20m.
- Hence, we can **reject the null hypothesis** (of it being randomly distributed) at 90% confidence interval and infer that its **clustering patterns are statistically significant**.
- In 2021, we can tell that the curve reaches L(r) - r above 400 while in 2019, the curve only reaches about 250. Hence, this shows that the clustering patterns actually increased in 2021.  

**Entire Home/Apt**

- For both years, the black line is well above the red line as well as the envelope after about 20m.
- Hence, we can **reject the null hypothesis** (of it being randomly distributed) at 90% confidence interval and infer that its **clustering patterns are statistically significant**.
- Additionally, both the curve reach about L(r) - r = 410. Hence, the clustering patterns are similar for both years.

**Shared Rooms**

- For both years, even though certain parts of the black line is above red line, it is not out of the envelope. 
- Hence, we **cannot reject the null hypothesis** at 90% confidence interval and infer that the the clustering patterns are **not** statistically significant.
- Additionally, in both years, the curve reach about L(r) - r = 500. However, in 2021, before the distance of 290m, it actually shows signs of regular patterns. But since it is not below the envelope, the **regular patterns are not statistically significant**.


# 9 Conclusion

## 9.1 Section A: Airbnb Distribution in 2019

- Based on the results, **signs of clustering patterns between Airbnb Listings with Hotels and MRT services** are  **statistically significant** only at certain distances.
- Whereas **signs of regular relationships between Airbnb Listings with Tourist Attractions** are  **statistically significant** at certain distances.


## 9.2 Section B: Impact of COVID-19

- **Entire Home/Apt** and **Shared Room** are found to be **not randomly distributed** and they have clustered patterns in both 2019 and 2021, with only **Entire Home/Apt** being **statistically significant**. In fact, the clustering patterns are similar for both years (based on L(r) - r).
- **Private Room** on the other hand shows an opposite where 2021 are found to have more clustered pattern than 2019.
- However, we must realise that these observations might not be conclusive due to limitations such as the amount of data available. Further analysis should still be done to confirm the results.


## 9.3 Summary

Airbnb listings in 2019 were found to be **NOT spatially independent** in the study area of Downtown Core with Hotels and MRT services and were found to have **significant clustered patterns at certain distances**. Airbnb listings were found to be **NOT spatially independent** with Tourists Attractions and were found to have **significant regular patterns at certain distances**.

Surprisingly, with COVID-19, Section B has revealed that it did not cause much of an impact on the distributions of Airbnb listings in 2021 when compared to 2019. With that being said, due to limitations such as the amount of data available, further analysis should still be performed to affirm the results. 


# 10 References

"density.ppp: Kernel Smoothed Intensity of Point Pattern" (n.d.)  Retrieved from:  https://www.rdocumentation.org/packages/spatstat/versions/1.64-1/topics/density.ppp

"How Kernel Density works" (n.d.) Retrieved from: https://pro.arcgis.com/en/pro-app/latest/tool-reference/spatial-analyst/how-kernel-density-works.htm

Manuel.G (2021) Chapter 11 Point Pattern Analysis. Retrieved from:  https://mgimond.github.io/Spatial/chp11_0.html#distance-based-analysis

Tin Seong.K (n.d.) Hands-on Exercise 4: Spatial Point Patterns Analysis-spatstat methods. Retrieved from: https://is415.netlify.app/hands-on_ex/hands-on_ex04/hands-on_ex04-sppa#second-order-spatial-point-patterns-analysis

Tin Seong.K (2021) Hands-on Exercise 5: Analysing Marked Point Patterns. Retrieved from: https://is415.netlify.app/hands-on_ex/hands-on_ex05/hands-on_ex05-markedsppa

Tin Seong.K (2021) Lesson 4: Spatial Point Patterns Analysis. Retrieved from:  https://is415.netlify.app/lesson/lesson04/lesson04-sppa#1
